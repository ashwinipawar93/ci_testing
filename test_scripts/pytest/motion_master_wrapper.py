...
class MotionMasterWrapper:
    ...
    def _construct_msg_for_signal_generator(self, profile_parameters, profile_type, device_address):

        if profile_type is 'position_bidirectional':
            ...
            position_bidirectional.target = profile_parameters['target']
            position_bidirectional.profile_velocity = profile_parameters['profile_velocity']
            position_bidirectional.profile_acceleration = profile_parameters['profile_acceleration']
            position_bidirectional.profile_deceleration = profile_parameters['profile_deceleration']
            position_bidirectional.sustain_time = profile_parameters['sustain_time']
            position_bidirectional.repeat = profile_parameters['does_repeat']
            position_bidirectional.master_generated = profile_parameters['is_master_generated']
            position_bidirectional.absolute_target = profile_parameters['is_absolute_target']

        elif profile_type is 'velocity_bidirectional':
            msg, obs = self._get_request_and_single_response_observable("SetSignalGeneratorParameters",
                                                                        timeout=REQUEST_RESPONSE_TIMEOUT)
            msg.request.set_signal_generator_parameters.device_address = device_address
            velocity_bidirectional = msg.request.set_signal_generator_parameters.velocity_bidirectional
            velocity_bidirectional.target = profile_parameters['target']
            velocity_bidirectional.profile_acceleration = profile_parameters['profile_acceleration']
            velocity_bidirectional.profile_deceleration = profile_parameters['profile_deceleration']
            velocity_bidirectional.sustain_time = profile_parameters['sustain_time']
            velocity_bidirectional.repeat = profile_parameters['does_repeat']
            velocity_bidirectional.master_generated = profile_parameters['is_master_generated']

        elif profile_type is 'position_ramp':

            msg, obs = self._get_request_and_single_response_observable("SetSignalGeneratorParameters",
                                                                        timeout=REQUEST_RESPONSE_TIMEOUT)
            msg.request.set_signal_generator_parameters.device_address = device_address
            position_ramp = msg.request.set_signal_generator_parameters.position_ramp
            position_ramp.target = profile_parameters['target']
            position_ramp.profile_velocity = profile_parameters['profile_velocity']
            position_ramp.profile_acceleration = profile_parameters['profile_acceleration']
            position_ramp.profile_deceleration = profile_parameters['profile_deceleration']
            position_ramp.sustain_time = profile_parameters['sustain_time']
            position_ramp.master_generated = profile_parameters['is_master_generated']
            position_ramp.absolute_target = profile_parameters['is_absolute_target']

        elif profile_type is 'velocity_ramp':
            msg, obs = self._get_request_and_single_response_observable("SetSignalGeneratorParameters",
                                                                        timeout=REQUEST_RESPONSE_TIMEOUT)
            msg.request.set_signal_generator_parameters.device_address = device_address
            velocity_ramp = msg.request.set_signal_generator_parameters.velocity_ramp
            velocity_ramp.target = profile_parameters['target']
            velocity_ramp.profile_acceleration = profile_parameters['profile_acceleration']
            velocity_ramp.profile_deceleration = profile_parameters['profile_deceleration']
            velocity_ramp.sustain_time = profile_parameters['sustain_time']
            velocity_ramp.master_generated = profile_parameters['is_master_generated']

        elif profile_type is 'position_sine_wave':
            msg, obs = self._get_request_and_single_response_observable("SetSignalGeneratorParameters",
                                                                        timeout=REQUEST_RESPONSE_TIMEOUT)
            msg.request.set_signal_generator_parameters.device_address = device_address
            position_sine_wave = msg.request.set_signal_generator_parameters.position_sine_wave
            position_sine_wave.amplitude = profile_parameters['amplitude']
            position_sine_wave.frequency = profile_parameters['frequency']
            position_sine_wave.absolute_target = profile_parameters['is_absolute_target']
            position_sine_wave.repeat = profile_parameters['does_repeat']

        elif profile_type is 'velocity_sine_wave':
            ...
            velocity_sine_wave.amplitude = profile_parameters['amplitude']
            velocity_sine_wave.frequency = profile_parameters['frequency']
            velocity_sine_wave.repeat = profile_parameters['does_repeat']

        elif profile_type is 'velocity_step_response':

            msg, obs = self._get_request_and_single_response_observable("SetSignalGeneratorParameters")
            msg.request.set_signal_generator_parameters.device_address = device_address
            velocity_step_response = msg.request.set_signal_generator_parameters.velocity_step_response
            velocity_step_response.target = profile_parameters['target']
            velocity_step_response.sustain_time = profile_parameters['sustain_time']

        elif profile_type is 'velocity_advanced_step_response':

            msg, obs = self._get_request_and_single_response_observable("SetSignalGeneratorParameters")
            msg.request.set_signal_generator_parameters.device_address = device_address
            velocity_advanced_step_response = msg.request.set_signal_generator_parameters.velocity_advanced_step_response
            velocity_advanced_step_response.target = profile_parameters['target']
            velocity_advanced_step_response.sustain_time = profile_parameters['sustain_time']
            velocity_advanced_step_response.repeat = profile_parameters['does_repeat']

        elif profile_type is 'position_advanced_step_response':

            msg, obs = self._get_request_and_single_response_observable("SetSignalGeneratorParameters")
            msg.request.set_signal_generator_parameters.device_address = device_address
            position_advanced_step_response = msg.request.set_signal_generator_parameters.position_advanced_step_response
            position_advanced_step_response.target = profile_parameters['target']
            position_advanced_step_response.sustain_time = profile_parameters['sustain_time']
            position_advanced_step_response.repeat = profile_parameters['does_repeat']
            position_advanced_step_response.absolute_target = profile_parameters['is_absolute_target']

        elif profile_type is 'velocity_trapezoidal':

            msg, obs = self._get_request_and_single_response_observable("SetSignalGeneratorParameters",
                                                                        timeout=REQUEST_RESPONSE_TIMEOUT)
            msg.request.set_signal_generator_parameters.device_address = device_address
            velocity_trapezoidal = msg.request.set_signal_generator_parameters.velocity_trapezoidal
            velocity_trapezoidal.target = profile_parameters['target']
            velocity_trapezoidal.profile_acceleration = profile_parameters['profile_acceleration']
            velocity_trapezoidal.profile_deceleration = profile_parameters['profile_deceleration']
            velocity_trapezoidal.sustain_time = profile_parameters['sustain_time']
            velocity_trapezoidal.repeat = profile_parameters['does_repeat']
            velocity_trapezoidal.master_generated = profile_parameters['is_master_generated']

        elif profile_type is 'position_trapezoidal':

            msg, obs = self._get_request_and_single_response_observable("SetSignalGeneratorParameters",
                                                                        timeout=REQUEST_RESPONSE_TIMEOUT)
            msg.request.set_signal_generator_parameters.device_address = device_address
            position_trapezoidal = msg.request.set_signal_generator_parameters.position_trapezoidal
            position_trapezoidal.target = profile_parameters['target']
            position_trapezoidal.profile_velocity = profile_parameters['profile_velocity']
            position_trapezoidal.profile_acceleration = profile_parameters['profile_acceleration']
            position_trapezoidal.profile_deceleration = profile_parameters['profile_deceleration']
            position_trapezoidal.sustain_time = profile_parameters['sustain_time']
            position_trapezoidal.repeat = profile_parameters['does_repeat']
            position_trapezoidal.master_generated = profile_parameters['is_master_generated']
            position_trapezoidal.absolute_target = profile_parameters['is_absolute_target']

        return msg, obs

    def _set_signal_generator_parameters_for_profile(self, profile_parameters, profile_type, device):

        device_address = device['info'].device_address
        device_position = device['info'].position

        for x in range(0, 2):
            msg, obs = self._construct_msg_for_signal_generator(profile_parameters, profile_type,
                                                               device_address)
            error_code = None
            success_code = None
            self.send_to_motion_master(msg)
            ...
            if response == 'success':
                success_code = MotionMasterMessage.Status.SignalGenerator.Success.Code.Name(
                    getattr(message.status.signal_generator, response).code)
                break
            elif response == 'error':
                error_code = MotionMasterMessage.Status.SignalGenerator.Error.Code.Name(
                    getattr(message.status.signal_generator, response).code)
                if error_code == 'ALREADY_RUNNING':
                    self.motion_master_stop_signal_generator(device)
                    logger.warning("For Device {} Signal generator was ALREADY RUNNING, Stop and "
                                   "make a second attempt to set :".format(device_position))
                    time.sleep(0.2)
                else:
                    # error is not 'ALREADY RUNNING' break.
                    break

        if error_code is not None:
            raise OperationFailed(
                "For device {}, signal generator returned error {}, while setting parameters"
                    .format(device_position, error_code))
        elif success_code != 'PARAMETERS_SET':
            raise OperationFailed("For device {}, signal generator parameters not set. Signal generator"
                                  " returned success code {}, while setting parameters"
                                  .format(device_position, success_code))

    def set_signal_generator_parameters_position_bidirectional(self, device, acceleration: int,
        ...
        # Check for error
        if sc.has_fault():
            error_description = od.error_report()
            raise OperationFailed("For device {}, signal generator cannot be set as drive has fault {}"
                                  .format(device['info'].position, error_description))
        else:

            profile_parameters = dict()
            profile_parameters['target'] = target_position
            profile_parameters['profile_velocity'] = profile_velocity
            profile_parameters['profile_acceleration'] = acceleration
            profile_parameters['profile_deceleration'] = deceleration
            profile_parameters['sustain_time'] = sustain_time
            profile_parameters['does_repeat'] = False
            profile_parameters['is_master_generated'] = True
            profile_parameters['is_absolute_target'] = False
            self._set_signal_generator_parameters_for_profile(profile_parameters, 'position_bidirectional', device)

    def set_signal_generator_parameters_velocity_bidirectional(self, device, acceleration: int,
        ...
        if sc.has_fault():
            ...
            profile_parameters = dict()
            profile_parameters['target'] = target
            profile_parameters['profile_acceleration'] = acceleration
            profile_parameters['profile_deceleration'] = deceleration
            profile_parameters['sustain_time'] = sustain_time
            profile_parameters['does_repeat'] = False
            profile_parameters['is_master_generated'] = True
            self._set_signal_generator_parameters_for_profile(profile_parameters, 'velocity_bidirectional', device)

    def set_signal_generator_parameters_position_ramp(self, device, acceleration: int,
                                                      deceleration: int, profile_velocity: int,
                                                      target_position: int, sustain_time: int):
        """
        Parameters
        ...
        if sc.has_fault():
            ...
            profile_parameters['is_master_generated'] = True
            profile_parameters['is_absolute_target'] = False
            self._set_signal_generator_parameters_for_profile(profile_parameters, 'position_ramp', device)

    def set_signal_generator_parameters_velocity_ramp(self, device, acceleration: int,
                                                      deceleration: int, target: int, sustain_time: int):
        ...
        if sc.has_fault():
            ...
            profile_parameters['sustain_time'] = sustain_time
            profile_parameters['is_master_generated'] = True
            self._set_signal_generator_parameters_for_profile(profile_parameters, 'velocity_ramp', device)

    def set_signal_generator_parameters_position_sine_wave(self, device, amplitude: int,
                                                      frequency: float):
        ...
        if sc.has_fault():
            ...
            profile_parameters = dict()
            profile_parameters['amplitude'] = amplitude
            profile_parameters['frequency'] = frequency
            profile_parameters['is_absolute_target'] = False
            profile_parameters['does_repeat'] = True
            self._set_signal_generator_parameters_for_profile(profile_parameters, 'position_sine_wave', device)

    def set_signal_generator_parameters_velocity_sine_wave(self, device: int, amplitude: int,
                                                      frequency: float):
        ...
        if sc.has_fault():
            ...
            profile_parameters['frequency'] = frequency
            profile_parameters['does_repeat'] = True
            self._set_signal_generator_parameters_for_profile(profile_parameters, 'velocity_sine_wave', device)
            
    def set_signal_generator_parameters_velocity_step_response(self, device,
                                                               target: int, sustain_time: int):

        """Set the parameters of the signal generator for step response
           Parameters
           ----------
           device
               Handler for the device
        ...
        if sc.has_fault():
            ...
            profile_parameters['target'] = target
            profile_parameters['sustain_time'] = sustain_time
            self._set_signal_generator_parameters_for_profile(profile_parameters, 'velocity_step_response', device)

    def set_signal_generator_parameters_velocity_advanced_step_response(self, device,
                                                                        target: int, sustain_time: int):
        """Set the parameters of the signal generator for step profile
           Parameters
           ----------
           device
               handler for the device
        ...
        if sc.has_fault():
            ...
            profile_parameters['sustain_time'] = sustain_time
            profile_parameters['does_repeat'] = True
            self._set_signal_generator_parameters_for_profile(profile_parameters, 'velocity_advanced_step_response', device)

    def set_signal_generator_parameters_position_advanced_step_response(self, device,
                                                                        target_position: int, sustain_time: int):
        """Set the parameters of the signal generator for step profile
           Parameters
           ----------
           device
               Handler for the device
           target_position : int
               target position of the profile
           sustain_time : int
               flat part of the trajectory at the target position (ms)
           Returns
           -------
        ...
        if sc.has_fault():
            ...
            profile_parameters['does_repeat'] = True
            profile_parameters['is_absolute_target'] = False
            self._set_signal_generator_parameters_for_profile(profile_parameters, 'position_advanced_step_response',
                                                              device)

    def set_signal_generator_parameters_velocity_trapezoidal(self, device, acceleration: int,
                                                             deceleration: int, target: int, sustain_time: int):
        """Set the parameters of the signal generator for step profile
           Parameters
           ----------
           device
               Handler for the device
           acceleration: int
               accerleration value for the profile
           deceleration: int
               decerleration value for the profile
           target : int
               target velocity of the profile
           sustain_time : int
               flat part of the trajectory at the target position (ms)
        ...
        if sc.has_fault():
            ...
            profile_parameters['is_master_generated'] = True
            profile_parameters['does_repeat'] = False
            self._set_signal_generator_parameters_for_profile(profile_parameters, 'velocity_trapezoidal', device)

    def set_signal_generator_parameters_position_trapezoidal(self, device, acceleration: int,
                                                             deceleration: int, target_position: int, sustain_time: int, profile_velocity: int):
        """Set the parameters of the signal generator for step profile
           Parameters
           ----------
           device
           ...
           sustain_time : int
               flat part of the trajectory at the target position (ms)
           profile_velocity : int
               velocity value to reach the target position
           Returns
           -------
               nothing
        """
        ...
        if sc.has_fault():
            ...
            profile_parameters['is_master_generated'] = True
            profile_parameters['is_absolute_target'] = False
            self._set_signal_generator_parameters_for_profile(profile_parameters, 'position_trapezoidal', device)

    def motion_master_start_signal_generator(self, device):
        ...
        if sc.has_fault():
            error_description = od.error_report()
            raise OperationFailed("For device {}, signal generator cannot be started as drive has fault {}"
                                  .format(device['info'].position, error_description))
        else:
            device_address = device['info'].device_address
            ...
            if response == 'error':
                ...
                raise OperationFailed("For device {}, signal generator returned error {}, while starting"
                                      .format(device['info'].position, error_code))
            elif response == 'success':
                success_code = MotionMasterMessage.Status.SignalGenerator.Success.Code.Name(
                    getattr(message.status.signal_generator, response).code)
                if success_code != 'STARTED':
                    raise OperationFailed("For device {}, signal generator did not start. Signal generator "
                                          "returned success code {}, while setting parameters"
                                          .format(device['info'].position, success_code))

    def motion_master_stop_signal_generator(self, device):
